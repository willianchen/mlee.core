...
using mlee.Core.Services.LoginLog;
...
public class AuthService : BaseService, IAuthService
{
    ...
    private string GetToken(AuthLoginOutput user)
    {
        ...
        var token = LazyGetRequiredService<ISession>().Create(new[]
        {
    ...
    public async Task<dynamic> LoginAsync(AuthLoginInput input)
    {
        using (_userRepository.DataFilter.DisableAll())
        {
            ...
            if (_appConfig.VarifyCode.Enable)
            {
                ...
                if (!isOk)
                {
                    throw ApiResult.ToException("安全验证不通过，请重新登录");
                }
            }
            ...
            if (input.PasswordKey.NotNull())
            {
                ...
                if (existsPasswordKey)
                {
                    ...
                    if (secretKey.IsNull())
                    {
                        throw ApiResult.ToException("解密失败");
                    }
            ...
            if (!(user?.Id > 0))
            {
                throw ApiResult.ToException("用户名或密码错误");
            }

            if (user.Status == UserStatus.Disabled)
            {
                throw ApiResult.ToException("禁止登录，请联系管理员");
            }
            #endregion
    ...
    public async Task<dynamic> Refresh([BindRequired] string token)
    {
        ...
        {
            throw ApiResult.ToException("无法解析token");
        }
        ...
        {
            throw ApiResult.ToException("验签失败");
        }
    ...
    public async Task CheckCaptcha([FromQuery] CaptchaInput input)
    {
        ...
        if (!check)
        {
            throw ApiResult.ToException("安全验证不通过");
        }
    }
}