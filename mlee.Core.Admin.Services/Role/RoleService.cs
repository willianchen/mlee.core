...
public class RoleService : BaseService, IRoleService, IDynamicApi
{
    ...
    /// <returns></returns>
    [HttpPost]
    public async Task<PageOutput<RoleGetPageOutput>> GetPageAsync(Pager<RoleGetPageDto> input)
    {
    ...
    public async Task<long> AddAsync(RoleAddInput input)
    {
        ...
        {
            throw ApiResult.ToException($"此{(input.Type == RoleType.Group ? "分组" : "角色")}已存在");
        }

        if (input.Code.NotNull() && await _roleRepository.Select.AnyAsync(a => a.ParentId == input.ParentId && a.Code == input.Code))
        {
            throw ApiResult.ToException($"此{(input.Type == RoleType.Group ? "分组" : "角色")}编码已存在");
        }
    ...
    public async Task UpdateAsync(RoleUpdateInput input)
    {
        ...
        if (!(entity?.Id > 0))
        {
            throw ApiResult.ToException("角色不存在");
        }
