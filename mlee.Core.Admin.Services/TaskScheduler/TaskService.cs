...
public class TaskService : BaseService, ITaskService, IDynamicApi
{
    ...
    /// <returns></returns>
    [HttpPost]
    public async Task<PageOutput<TaskListOutput>> GetPageAsync(Pager<TaskGetPageDto> input)
    {
    ...
    public string Add(TaskAddInput input)
    {
        ...
        {
            throw ApiResult.ToException("请输入定时参数");
        }
    ...
    public async Task UpdateAsync(TaskUpdateInput input)
    {
        ...
        {
            throw ApiResult.ToException("任务不存在！");
        }
