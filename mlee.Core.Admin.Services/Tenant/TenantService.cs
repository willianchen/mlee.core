...
public class TenantService : BaseService, ITenantService, IDynamicApi
{
    ...
    /// <returns></returns>
    [HttpPost]
    public async Task<PageOutput<TenantListOutput>> GetPageAsync(Pager<TenantGetPageDto> input)
    {
    ...
    public virtual async Task<long> AddAsync(TenantAddInput input)
    {
        ...
        {
            throw ApiResult.ToException($"企业名称已存在");
        }

        if (await _tenantRepository.Select.AnyAsync(a => a.Code == input.Code))
        {
            throw ApiResult.ToException($"企业编码已存在");
        }
    ...
    public async Task UpdateAsync(TenantUpdateInput input)
    {
        ...
        if (!(entity?.Id > 0))
        {
            throw ApiResult.ToException("租户不存在！");
        }
    ...
    public virtual async Task DeleteAsync(long id)
    {
        ...
        {
            throw ApiResult.ToException("平台租户禁止删除");
        }
