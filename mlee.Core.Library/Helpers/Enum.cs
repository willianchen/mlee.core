...
namespace mlee.Core.Library.Helpers
{
    ...
    public static partial class Enum
    {
        ...
        /// <summary>
        /// 获取描述,使用NoteAttribute特性设置描述
        /// </summary>
        /// <param name="type">枚举类型</param>
        /// <param name="member">成员名、值、实例均可</param>
        public static string GetNote(Type type, object member)
        {
            return Reflection.GetNote(type, GetName(type, member));
        }
        ...
        /// <param name="member">成员名、值、实例均可</param>
        /// <returns></returns>
        public static string GetNote<TEnum>(object member)
        {
            return Reflection.GetNote<TEnum>(GetName<TEnum>(member));
        }
        /// <summary>
        ...
        /// <summary>
        /// 获取项集合,文本设置为Note，值为Value
        /// </summary>
        /// <typeparam name="TEnum">枚举类型</typeparam>
        public static List<Item> GetItemsByNote<TEnum>()
        {
            return GetItems(typeof(TEnum), 2);
        }
        ...
        /// </summary>
        /// <param name="type">枚举类型</param>
        /// <param name="desOrNote">取值（1Des 2Note）</param>
        public static List<Item> GetItems(Type type, int desOrNote = 1)
        {
            ...
            foreach (var field in type.GetFields())
                AddItem(type, result, field, desOrNote);
        ...
        /// 添加描述项
        /// </summary>
        private static void AddItem(Type type, ICollection<Item> result, FieldInfo field, int desOrNote = 1)
        {
            ...
            if (desOrNote == 1)
                description = Reflection.GetDescription(field);
            else if (desOrNote == 2)
                description = Reflection.GetNote(field);

            result.Add(new Item(description, value, name, value));
        }
