...
namespace mlee.Core.Library.Webs.Clients
{
    /// <summary>
    /// Http请求
    /// </summary>
    public abstract class HttpRequestBase<TRequest> where TRequest : IRequest<TRequest>
    {
       ...
       // private readonly HttpClient _httpClient;
        ...
        /// <returns></returns>
        public TRequest Get(string url)
        {
            if (string.IsNullOrWhiteSpace(url))
                throw new ArgumentNullException(nameof(url));
            _url = url;
        ...
        /// <param name="url"></param>
        /// <returns></returns>
        public TRequest Post(string url)
        {
            if (string.IsNullOrWhiteSpace(url))
                throw new ArgumentNullException(nameof(url));
            _url = url;
        ...
        /// <returns></returns>
        public virtual TRequest SetHuntNetAuth(string appName, string appSecret)
        {
            var ticikes = DateTime.Now.Ticks;
            var token = $"{appName}{appSecret}{ticikes}";
            var encryptToken = Md5Encrypt.Encrypt($"{token}");

            var authorization = $"HuntNetAuth {encryptToken}:{ticikes}";
            return Header("Token", authorization);
        }
        ...
        /// <returns></returns>
        public virtual TRequest SetBasicAuth(string userName, string pwd)
        {
            var token = System.Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes($"{userName}:{pwd}"));
        ...
        protected virtual void SendBefore()
        {
            ...
            {
                token = _authenticationHeader.Scheme + ":" + _authenticationHeader.Parameter;
            }
            var headJson = _headers.Count > 0 ? _headers.ToJson() : "";
            logger.Trace($"请求：{_url}|{headJson}|{token}|{_data}");
        }
        ...
        protected virtual void InitHttpClient(HttpClient client)
        {
            ...
            if (_authenticationHeader != null)
            {
                ...
                //client.SetBasicAuthentication(_userName, _pwd);
            }
            else
            {
