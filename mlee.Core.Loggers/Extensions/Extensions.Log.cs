    ...
    public static partial class Extensions
    {
        ...
        public static ILog Params(this ILog log, string name, string value, string type = null)
        {
            return log.Set<LogContent>(content =>
            {
                ...
                {
                    content.AppendLine(content.Params, $"ParameterName: {name}, ParameterValue: {value}");
                    return;
                }
                content.AppendLine(content.Params, $"ParameterType: {type}, ParameterName: {name},ParameterValue: {value}");
            });
        }
        ...
        public static ILog Params(this ILog log, IDictionary<string, object> dictionary)
        {
            ...
            foreach (var item in dictionary)
                Params(log, item.Key, item.Value.ToSafeString());
            return log;
        }
        ...
        public static ILog SqlParams(this ILog log, IEnumerable<KeyValuePair<string, object>> list)
        {
            ...
            return SqlParams(log, Library.Helpers.String.RemoveEnd(result.ToString(), ($",{Common.Line}")));
        }
        ...
        private static string GetParamLiterals(object value)
        {
            ...
            {
                case "boolean":
                    return Library.Helpers.Convert.ToBool(value) ? "1" : "0";
                case "int16":
                case "int32":
                case "int64":
                case "single":
