...
namespace mlee.Core.Redis.RedisManage
{
    ...
    public class RedisOperationHelp : IRedisOperationHelp
    {
        ...
        /// <returns></returns>
        public async Task ListClearAsync(string key, long start, long stop)
        {
            await redisBase.DoSave(db => db.ListTrimAsync(ListSysCustomKey + key, start, stop));
        }
        /// <summary>
        /// 存储list 集合
        /// </summary>
        ...
        public async Task<long> ListRightPushAsync(string key, string value)
        {
            if (string.IsNullOrWhiteSpace(value))
                throw new ApplicationException("值不能为空");
            return await redisBase.DoSave(db => db.ListRightPushAsync(ListSysCustomKey + key, value));
        }
        /// <summary>
        /// 删除并返回存储在key上的列表的第一个元素。
        /// </summary>
        ...
        public async Task<long> ListLeftPushAsync<T>(string key, T value)
        {
            if (value == null)
                throw new ApplicationException("值不能为空");
            return await redisBase.DoSave(db => db.ListLeftPushAsync(ListSysCustomKey + key, redisBase.ConvertJson(value)));
        }
        /// <summary>
        /// 往最后推送一个数据
        /// </summary>
        ...
        public async Task<long> ListLeftPushAsync<T>(string key, List<T> value)
        {
            ...
            return await redisBase.DoSave(db => db.ListLeftPushAsync(ListSysCustomKey + key, redisValues.ToArray()));
        }
        /// <summary>
        /// 往末尾推送多条数据
        /// </summary>
        ...
        public async Task<long> ListLeftPushAsync(string key, string[] value)
        {
            if (value == null || value.Count() <= 0)
                throw new ApplicationException("值不能为空");
            return await redisBase.DoSave(db => db.ListLeftPushAsync(ListSysCustomKey + key, value.ToRedisValueArray()));
        }
        /// <summary>
        /// 往末尾推送多条数据
        /// </summary>
        ...
        /// <returns></returns>
        public async Task<bool> KeyExpireAsync(string key, TimeSpan? expiry = default(TimeSpan?), KeyOperatorEnum keyOperatorEnum = default)
        {
            if (keyOperatorEnum == KeyOperatorEnum.String)
            {
                key = StringSysCustomKey + key;
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public async Task<List<T>> SortedSetGetListAsync<T>(string key, double score)
        {
            var result = await redisBase.DoSave(db => db.SortedSetRangeByScoreWithScoresAsync(SortedSetCustomKey + key, score));
            return redisBase.ConvertObj<List<T>>(result.ToJson());
        }
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public async Task<bool> SortedSetIncrementAsync<T>(string key, T value)
        {
            var val = redisBase.ConvertJson(value);

            var result = await redisBase.DoSave(db => db.SortedSetIncrementAsync(SortedSetCustomKey + key, val, 1));
            return result > 0;
        }
        ...
        {
            var vList = await redisBase.DoSave(db => db.SetMembersAsync(SetSysCustomKey + key));
            List<T> result = new List<T>();
            foreach (var item in vList)
            {
                var model = redisBase.ConvertObj<T>(item); //反序列化
        ...
        {
            var vList = redisBase.DoSave(db => db.SetMembers(SetSysCustomKey + key));
            List<T> result = new List<T>();
            foreach (var item in vList)
            {
                var model = redisBase.ConvertObj<T>(item); //反序列化
        ...
        public async Task<long> SetAddAsync<T>(string key, List<T> value)
        {
            ...
            return await redisBase.DoSave(db => db.SetAddAsync(SetSysCustomKey + key, redisValues.ToArray()));
        }
        /// <summary>
        /// 获取Set集合元素个数
        /// </summary>
        ...
        {
            return await redisBase.DoSave(db => db.SetLengthAsync(SetSysCustomKey + key));
        }
        /// <summary>
        /// 移除
        /// </summary>
